...
const GroupPoints: React.FC = () => {
  ...
  // Handler to load points from global context (fetched points)
  const handleLoadFetchedPoints = () => {
    if (globalState.points && globalState.points.length > 0) {
      // Map global points to local Point type, preserving all BACnet data
      const mappedPoints = globalState.points.map((point, index) => ({
        ...
        objectInst: point.objectInst ? String(point.objectInst) : '',
        objectIdentifier: point.objectIdentifier || '',
        otDeviceInst: point.otDeviceInst ? String(point.otDeviceInst) : '',
        deviceInstance: point.deviceInst || '',
        deviceAddress: point.deviceAddr || '',
        presentValue: point.presentValue || '',
        unit: point.unit || '',
        covIncrement: point.covIncrement || '',
        description: point.description || '',
    ...
    <div className="group-points-page">
      ...
      <Card className="upload-card">
        ...
        <div className="file-upload-container">
          ...
          <button
            type="button"
            className="load-fetched-points-button"
            onClick={handleLoadFetchedPoints}
            disabled={globalState.points.length === 0}
          >
            Load Points from Fetch
          </button>
        </div>
