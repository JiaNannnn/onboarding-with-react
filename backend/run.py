...
# Add the backend1 directory to the Python path to import BMSDataPoints
backend1_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'backend1')
if os.path.exists(backend1_dir):
    sys.path.append(backend1_dir)
    from data.BMSDataPoints import discover_devices, get_points_for_devices, getNetConfig, searchDevice, fetchPoints, search_points
else:
    print(f"Warning: Could not find backend1 directory at {backend1_dir}")
